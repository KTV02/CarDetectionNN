...
downloadPath="C:/carpictures/"
driverPath="C:/Users/Kremp/Desktop/urlaub2020/chromedriver.exe"
#############################################################################	
...
def formatTitle(title):
    ...
    if "citrön" in title:
        title=title.replace("citrön","citroen")
    return title
########################################################################################
...
def setListings(driver):
    #dieser link ist in jedem link der auf eine anziege zeigt vorhanden, daher werden diese herausgefiltert
    listings=driver.find_elements_by_xpath("//*[contains(@href, 'https://suchen.mobile.de/fahrzeuge/details.html')]")
    return listings
    ...
    for i in range(len(links)):
       ...
       # driver.execute_script("arguments[0].scrollIntoView();", currentlisting)
        ...
        if(len(mostSimilarModels)>0):
            ...
            with open("noMatches.txt", "a") as f:
                f.write("\n"+ title)
...
def main():
    ...
    for f in range(len(listings)):
        links.append(listings[f].get_attribute("href"))
